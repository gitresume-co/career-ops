...
## Paso 3 — Generate tailored gitresume.yaml and push to GitResume
Execute the full `pdf` pipeline (read `modes/pdf.md`). This generates a `gitresume.yaml` tailored to the JD and pushes it to a new branch on the user's GitResume repo. GitResume auto-builds the PDF.
