...
| `templates/resume.schema.json` | GitResume YAML schema (source of truth for `resume.yaml`) |
| `templates/cv-template.tex` | LaTeX/Overleaf template (only if `cv.output_format: latex`) |
| `generate-latex.mjs` | LaTeX CV validator + pdflatex compiler (LaTeX path only) |
...
- Node.js (mjs modules), Playwright (offer verification + scraping), YAML (config), Markdown (data), Git (GitResume integration)
